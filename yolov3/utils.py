...
#
#   File name   : utils.py
#   Author      : PyLessons, Editted by Ryan Werth
#   Created date: 2020-09-27 / 2020-12-09
...
# I created this function to import my custom weights
# This uses the custom weights that are in model_data/custom
def Load_Custom_Yolo_model():
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE, CLASSES=YOLO_CUSTOM_CLASSES)

        load_yolo_weights(yolo, YOLO_V3_CUSTOM_WEIGHTS) # use custom weights
...
def draw_bbox(image, bboxes, color_ratios, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if color_ratio <= .2:
            bbox_color = (50, 255, 255)
            team_name = 'Team Light'
        else :
            bbox_color = (255,0,0)
            team_name = 'Team Dark'
        ...
        cv2.rectangle(image, (x1, y1), (x2, y2), bbox_color, int(bbox_thick*2))
        ...
        if show_label:
            ...
            # label = "{}".format( team_name + str(round(color_ratio,3)))
